...
export default class Start extends Command {
    ...
    async run() {
        ...
        if (flags.job) {
            ...
            const template = Handlebars.compile(readFileSync(projectConfig.jobTemplatePath).toString(), { strict: true })
            ...
            zoweSync(`zos-uss issue ssh "${projectConfig.shellStartCommand}" --cwd "${userConfig.zosTargetDir}"`, { direct: true })
        }
    }
}
